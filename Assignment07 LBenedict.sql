...
Begin Try
	Use Master;
	If Exists(Select Name From SysDatabases Where Name = 'Assignment07DB_LBenedict')
	 Begin 
	  Alter Database [Assignment07DB_LBenedict] set Single_user With Rollback Immediate;
	  Drop Database Assignment07DB_LBenedict;
	 End
	Create Database Assignment07DB_LBenedict;
End Try
Begin Catch
	Print Error_Number();
End Catch
go
Use Assignment07DB_LBenedict;
...
SELECT
	 c.CategoryName
	,p.ProductName
	,UnitPrice = Format(p.UnitPrice, 'c','en-us') -- format as USD
FROM
	vCategories as c
...
Order By
	CategoryName, MONTH(InventoryDate), InventoryCountByCategory;
go
...
FROM
	vProductInventories
ORDER BY
	1,CAST(InventoryDate AS DATE),3;
go
